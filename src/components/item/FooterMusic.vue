...
import MusicDetail from '@/components/item/MusicDetali.vue'
...
export default {
    ...
    mounted(){
        // console.log("播放标签属性---", this.$refs)
        this.$store.dispatch("getLyric", this.playList[this.playListIndex].id)
        this.updateTime()   // 在外部同样需要更新歌曲播放时间
    },
    updated(){  // 网页元素发生变化时就发送请求
        this.$store.dispatch("getLyric", this.playList[this.playListIndex].id)
        this.addDuration()
    },
    methods:{
        ...
        updateTime:function(){  //  定时器  设定每秒钟更新一次歌曲播放时间      ( audio标签里timeupdate事件也可 )
    ...
    watch:{
        playListIndex:function(){   // 监听播放歌曲下标是否发生改变
            this.$refs.audio.autoplay = true
            if(this.$refs.audio.paused){
                this.updateIsbtnShow(false)
            }
        },
        playList:function(){    // 解决初始状态播放第一首时歌曲下标未发生改变的情况
            if(this.isbtnShow){
                this.$refs.audio.autoplay = true;
                this.updateIsbtnShow(false)
            }
